...
export const DocumentsBlock = ({ item }: { item: BlockDocuments }): React.JSX.Element => {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16">
      ...
      {isLoading && <p aria-live="polite" className="text-sm text-muted-foreground" role="status">Dokumente werden geladen ...</p>}
      {!isLoading && hasError && <EmptyState message="Dokumente konnten nicht geladen werden." />}
      {!isLoading && !hasError && documents.length === 0 && <EmptyState message="Keine Dokumente vorhanden." />}
      {!isLoading && !hasError && documents.length > 0 && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {documents.map((document) => {
            ...
            return (
              <article className="border border-border bg-card p-6" key={document.id}>
                <h3 className="text-xl font-semibold">{document.title ?? ""}</h3>
                {fileId && directusUrl && (
                  <a className="mt-4 inline-block text-sm underline underline-offset-4" href={`${directusUrl}/assets/${fileId}`} target="_blank" rel="noreferrer">
                    Dokument oeffnen
                  </a>
                )}
              </article>
